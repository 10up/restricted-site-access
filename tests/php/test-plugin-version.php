...
class PluginVersionTests extends TestCase {
	...
	public static function get_plugin_version_constant() {
		$plugin_file_name = 'restricted_site_access.php';

		/*
		 * Determine the value of `RSA_VERSION` constant.
		 *
		...
		if ( preg_match( '/define\(\s*[\'"]RSA_VERSION[\'"]\s*,\s*[\'"]([^\'"]+)[\'"]\s*\)/', $plugin_file_contents, $matches ) ) {
			return $matches[1];
		}
